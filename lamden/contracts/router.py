...
def mint(ethereum_contract: str, amount: str, lamden_wallet: str):
    assert (
        ctx.caller == owner.get()
    ), f"Only owner can call! Current caller is {ctx.caller}, owner should be {owner.get()}"
...
@export
def burn(
    ethereum_contract: str, ethereum_address: str, lamden_address: str, amount: float
):
